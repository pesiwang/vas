...
void EchoObserver::onTimer(){
	...
	for(map<RoleService*, time_t>::iterator iter = this->_services.begin(); iter != this->_services.end();){
...
void EchoObserver::onListenerRegistered(RoleListener* listener){
	VAS_LOGGER_DEBUG("onListenerRegistered, listener = %d", listener->getFd());
}

void EchoObserver::onListenerUnregistered(RoleListener* listener){
	VAS_LOGGER_DEBUG("onListenerUnregistered, listener = %d", listener->getFd());
	delete listener;
}

void EchoObserver::onServiceRegistered(RoleService* service){
...
void EchoObserver::onServiceDataSent(RoleService* service){
